...
function initRound2IDE() {
    ...
    require(['vs/editor/editor.main'], function() {
        editor = monaco.editor.create(document.getElementById('monaco-editor-container'), {
            ...
            language: questions[0]?.language || 'java',
            theme: 'vs-dark',
...
function loadQuestion(index) {
    ...
    updateErrorMeter(q.errors_fixed_count || 0, q.total_errors || 7);
...
function updateErrorMeter(fixedCount, totalErrors, statusList = null) {
    const counterElem = document.getElementById('errors-fixed-counter');
    if (counterElem) {
        counterElem.textContent = `${fixedCount} / `;
    }
...
function updateNavigatorUI() {
    questions.forEach((q, idx) => {
        ...
        } else if (q.errors_fixed_count === (q.total_errors || 7)) {
...
function setupEventListeners() {
    ...
    document.getElementById('btn-run').addEventListener('click', () => {
        ...
        outElem.innerHTML = '<span class="text-indigo-400 animate-pulse">Analyzing source for 7 error checkpoints...</span>';
        ...
        .then(data => {
            ...
            updateErrorMeter(fixedCount, q.total_errors || 7, data.fixed_status_list);

            let statusHtml = `
                <div class="mb-3 p-3 rounded bg-slate-900/80 border border-slate-700">
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-sm font-semibold text-slate-200">Analysis Report</span>
                        <span class="px-2 py-0.5 rounded text-xs font-bold ${fixedCount === (q.total_errors || 7) ? 'bg-emerald-950 text-emerald-400 border border-emerald-500/40' : 'bg-indigo-950 text-indigo-400 border border-indigo-500/40'}">
                            ${fixedCount} of  Errors Corrected
                        </span>
                    </div>
    ...
    document.getElementById('btn-submit').addEventListener('click', () => {
        ...
        const code = editor ? editor.getValue() : q.current_code;
        const outElem = document.getElementById('output-terminal');
        outElem.innerHTML = '<span class="text-indigo-400 animate-pulse">Submitting solution & calculating score...</span>';
        ...
        .then(data => {
            ...
            q.errors_fixed_count = fixedCount;
            q.score = data.score;
            updateErrorMeter(fixedCount, q.total_errors || 7);

            if (fixedCount === (q.total_errors || 7)) {
                outElem.innerHTML = `
                    <div class="p-3 bg-emerald-950/60 border border-emerald-500/50 rounded-lg text-emerald-300">
                        <div class="flex items-center gap-2 font-bold text-sm mb-1">
                            ...
                            OUTSTANDING! All 7 Errors Fixed (+${data.score} Marks)
                        </div>
                ...
                outElem.innerHTML = `
                    <div class="p-3 bg-amber-950/60 border border-amber-500/50 rounded-lg text-amber-300">
                        <div class="flex items-center gap-2 font-bold text-sm mb-1">
                            ...
                            Partial Solution: ${fixedCount}/ Errors Fixed (+${data.score} Marks)
                        </div>
